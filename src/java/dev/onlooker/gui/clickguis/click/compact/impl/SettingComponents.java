...
public void handle(int mouseX, int mouseY, int button, GuiEvents type) {
    if (HUDMod.watermarkTheme.is("Light")) {
        categoryColor = new Color(205, 205, 205, 100);
    }else {
        categoryColor = new Color(0, 0, 0, 100);
    }
    typing = false;
    float settingHeight = 16;
    float count = 0;
    Color accentColor = actualColor;
    Color disabledColor = categoryColor;
    for (Setting setting : module.getSettingsList()) {
        ...
        if (setting instanceof ColorSetting) {
            ...
            if (clickAnimation.isDone() && clickAnimation.getDirection().equals(Direction.FORWARDS) || !clickAnimation.isDone()) {
                ...
                // Hue bar
                RenderUtil.color(textColor);
                mc.getTextureManager().bindTexture(new ResourceLocation("OnLooker/Images/hue2.png"));

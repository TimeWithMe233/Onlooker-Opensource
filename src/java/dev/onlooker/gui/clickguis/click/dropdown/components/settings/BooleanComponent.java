...
public class BooleanComponent extends SettingComponent<BooleanSetting> {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        RenderUtil.resetColor();
        RoundedUtil.drawRound(booleanX, booleanY, switchWidth, switchHeight, ClickGUIMod.radius.getValue().floatValue() + 1, rectColor);

        RenderUtil.resetColor();

        RoundedUtil.drawRound(x + width - (switchWidth + 4) + ((switchWidth - 8) * toggleAnimation.getOutput().floatValue()),
                y + tenacityFont16.getMiddleOfBox(height) + .5f, 5, 5, ClickGUIMod.radius.getValue().floatValue(),textColor);
    }

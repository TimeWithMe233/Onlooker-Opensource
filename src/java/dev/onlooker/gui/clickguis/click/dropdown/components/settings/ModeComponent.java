package dev.onlooker.gui.clickguis.click.dropdown.components.settings;

import dev.onlooker.gui.clickguis.click.dropdown.components.SettingComponent;
...
import dev.onlooker.utils.tuples.mutable.MutablePair;
...
public class ModeComponent extends SettingComponent<ModeSetting> {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        RoundedUtil.drawRound(boxX, boxY, boxWidth, boxHeight, ClickGUIMod.radius.getValue().floatValue(), outlineColor);


        RoundedUtil.drawRound(boxX + 1, boxY + 1, boxWidth - 2, boxHeight - 2, ClickGUIMod.radius.getValue().floatValue(), rectColor);
        ...
        if(themeSetting){
            ...
            RoundedUtil.drawRound(boxX + 7.5f + tenacityFont16.getStringWidth(modeSetting.getMode()), boxY + middleOfRect,
                    width, height, ClickGUIMod.radius.getValue().floatValue(), themeColors.getFirst());
            RoundedUtil.drawRound(boxX + 7.5f + tenacityFont16.getStringWidth(modeSetting.getMode()) + (spacing + width), boxY + middleOfRect,
                    width, height, ClickGUIMod.radius.getValue().floatValue(), themeColors.getSecond());
        }
        ...
        if (opened || !openAnimation.isDone()) {
            ...
            RoundedUtil.drawRound(modeX, modeY, boxWidth, Math.max(4, modeHeight * openAnim), ClickGUIMod.radius.getValue().floatValue(),
            ...
            RoundedUtil.drawRound(modeX + 1.5f, (float) (modeY + 1.5f + selectionBoxY.getOutput()), boxWidth - 3, rectHeight - 3, ClickGUIMod.radius.getValue().floatValue(),

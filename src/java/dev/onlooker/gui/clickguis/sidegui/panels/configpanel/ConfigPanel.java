...
import dev.onlooker.gui.clickguis.sidegui.utils.ActionButton;
import dev.onlooker.gui.clickguis.sidegui.utils.CarouselButtons;
import dev.onlooker.gui.clickguis.sidegui.utils.DropdownObject;
...
public class ConfigPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (voteRefreshTimer.hasTimeElapsed(30000)) {
            Multithreading.runAsync(() -> Client.INSTANCE.getCloudDataManager().refreshVotes());
            voteRefreshTimer.reset();
        }


        Client.INSTANCE.getCloudDataManager().applyVotes();
                    ...
                    case "Upload config":
                        ...
                        uploadForm.setTriUploadAction((name, description, server) -> {
                            Multithreading.runAsync(() -> {
                                String fileData = Client.INSTANCE.getConfigManager().serialize();
        ...
        switch (carouselButtons.getCurrentButton()) {
            case "Cloud":
                ...
                if (!sortingSelection.equals(sorting.getSelection())) {
                    Multithreading.runAsync(() -> Client.INSTANCE.getCloudDataManager().refreshData());
                    sortingSelection = sorting.getSelection();
                }
                ...
                for (CloudConfigRect cloudConfigRect : cloudConfigRects) {
                    if (filterVersion && !cloudConfigRect.getConfig().getVersion().equals(Client.VERSION)) continue;
    ...
    public void refresh() {
        ...
        for (CloudConfig cloudConfig : Client.INSTANCE.getCloudDataManager().getCloudConfigs()) {
    ...
    public Comparator<CloudConfigRect> relevanceSorting() {
        return Comparator.<CloudConfigRect>comparingInt(configRect -> {
            ...
            versionScore = cloudConfig.getVersion().equals(Client.VERSION) ? 100 : 0;

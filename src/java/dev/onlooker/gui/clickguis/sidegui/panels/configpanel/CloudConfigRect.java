...
import dev.onlooker.gui.clickguis.sidegui.forms.impl.EditForm;
...
public class CloudConfigRect implements Screen {
    ...
    public CloudConfigRect(CloudConfig config) {
        this.config = config;
        Client.INSTANCE.getSideGui().getTooltips().add(hoverInformation);
            ...
            iconButton.setClickAction(() -> {
                switch (iconButton.getIcon()) {
                    case FontUtil.LOAD:
                        ...
                        break;
                    case FontUtil.EDIT:
                        Form form = Client.INSTANCE.getSideGui().displayForm("Edit Config");
                        ((EditForm) form).setup(config, false);
                        form.setUploadAction((fileName, updatedDescription) -> {
                            Multithreading.runAsync(() -> {

                                String data = Client.INSTANCE.getConfigManager().serialize();

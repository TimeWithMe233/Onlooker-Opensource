...
public final class RotationComponent extends ListenerAdapter {
    ...
    public void onMoveInput(EventMoveInput event) {
        if (modify && movementFix && !strict && (KillAura.target == null || !Client.INSTANCE.getModuleCollection().getModule(Speed.class).shouldFollow())) {
            ...
            for (float predictedForward = -1.0f; predictedForward <= 1.0f; predictedForward += 1.0f) {
                for (float predictedStrafe = -1.0f; predictedStrafe <= 1.0f; predictedStrafe += 1.0f) {
                    ...
                    double difference;
                    if (predictedStrafe == 0.0f && predictedForward == 0.0f || !((difference = Math.abs(angle - (predictedAngle = MathHelper.wrapAngleTo180_double(Math.toDegrees(RotationComponent.getDirection(yaw, predictedForward, predictedStrafe)))))) < (double) closestDifference))
                        continue;

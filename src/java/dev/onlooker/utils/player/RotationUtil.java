...
public class RotationUtil{
    ...
    public Vector2f calculate(final dev.onlooker.utils.vector.Vector3d from, final dev.onlooker.utils.vector.Vector3d to) {
        final dev.onlooker.utils.vector.Vector3d diff = to.subtract(from);
    ...
    public static float[] getRotationBlock(BlockPos pos) {
        return RotationUtil.getRotationsByVec(mc.thePlayer.getPositionVector().addVector(0.0, mc.thePlayer.getEyeHeight(), 0.0), new Vec3((double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5));
    }

    public static EnumFacing calculateFacing(EntityPlayer player, BlockPos target) {
        ...
        if (absX > absY && absX > absZ) {
            return diff.xCoord > 0 ? EnumFacing.WEST : EnumFacing.EAST;
        } else if (absY > absX && absY > absZ) {
            return diff.yCoord > 0 ? EnumFacing.DOWN : EnumFacing.UP;
        } else {
            return diff.zCoord > 0 ? EnumFacing.NORTH : EnumFacing.SOUTH;
        }
    }

    public static dev.onlooker.utils.vector.Vector2f applySensitivityPatchA(final dev.onlooker.utils.vector.Vector2f rotation, final dev.onlooker.utils.vector.Vector2f previousRotation) {
        ...
        return new dev.onlooker.utils.vector.Vector2f(yaw, MathHelper.clamp_float(pitch, -90, 90));
    }
    ...
    public Vector2f calculate(final Vec3 to, final EnumFacing enumFacing) {
        return calculate(new dev.onlooker.utils.vector.Vector3d(to.xCoord, to.yCoord, to.zCoord), enumFacing);
    }

    public Vector2f calculate(final Vec3 to) {
        return calculate(mc.thePlayer.getCustomPositionVector().add(0, mc.thePlayer.getEyeHeight(), 0), new dev.onlooker.utils.vector.Vector3d(to.xCoord, to.yCoord, to.zCoord));
    }

    public Vector2f calculate(final dev.onlooker.utils.vector.Vector3d to) {
    ...
    public Vector2f calculate(final dev.onlooker.utils.vector.Vector3d position, final EnumFacing enumFacing) {
        ...
        return calculate(new dev.onlooker.utils.vector.Vector3d(x, y, z));
    }

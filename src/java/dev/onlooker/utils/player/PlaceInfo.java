...
public final class PlaceInfo {
    ...
    public static PlaceInfo get(BlockPos blockPos) {
        if (BlockUtils.isValidBock(blockPos.add(0, -1, 0))) {
            return new PlaceInfo(blockPos.add(0, -1, 0), EnumFacing.UP, null, 4);
        }
        if (BlockUtils.isValidBock(blockPos.add(0, 0, 1))) {
            return new PlaceInfo(blockPos.add(0, 0, 1), EnumFacing.NORTH, null, 4);
        }
        if (BlockUtils.isValidBock(blockPos.add(-1, 0, 0))) {
        ...
        return BlockUtils.isValidBock(blockPos.add(1, 0, 0)) ? new PlaceInfo(blockPos.add(1, 0, 0), EnumFacing.WEST, null, 4) : null;
    }

...
import dev.onlooker.module.impl.display.PostProcessing;
import dev.onlooker.module.impl.display.ScoreboardMod;
...
public class GuiIngame extends Gui implements Utils {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Client.INSTANCE.getEventProtocol().handleEvent(new Render2DEvent(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(),scaledresolution));
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            drawTexturedModalRect(i - 91 - 1 + RenderSlotComponent.getSpoofedSlot() * 20, (float) (sr.getScaledHeight() - (22 + 15 * GuiChat.openingAnimation.getOutput().floatValue())), 0, 22, 24, 22);
...
//            RoundedUtil.drawRound(l1 - 2, k, l, k + this.getScoreboardFontRenderer().getHeight(),8.0f, new Color(color.getRGB()));
            ...
            // Line text
            if(ScoreboardMod.customFont.isEnabled()) {
                FontManager.Naven18.drawString(s1, l1, k, -1, ScoreboardMod.textShadow.isEnabled());
            }else {
            ...
            if (ScoreboardMod.redNumbers.isEnabled()) {
                ...
                if(ScoreboardMod.customFont.isEnabled()) {
                FontManager.Naven18.drawString(s2, l - this.getScoreboardFontRenderer().getStringWidth(s2), k, -1, ScoreboardMod.textShadow.isEnabled());
            }else {
                    FontManager.Naven18.drawString(s2, l - this.getScoreboardFontRenderer().getStringWidth(s2), k, -1, ScoreboardMod.textShadow.isEnabled());
                }
            }
...
//                RoundedUtil.drawRound(l1 - 2, k - 1, l, k,8.0f,new Color( color.getRGB()));
                if (ScoreboardMod.customFont.isEnabled()) {
                    FontManager.Naven18.drawString(s3, l1 + i / 2.0F - this.getScoreboardFontRenderer().getStringWidth(s3) / 2.0F, k - this.getScoreboardFontRenderer().getHeight(), -1, ScoreboardMod.textShadow.isEnabled());
                }else {

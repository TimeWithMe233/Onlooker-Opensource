...
import dev.onlooker.utils.addons.viamcp.vialoadingbase.netty.event.CompressionReorderEvent;
import dev.onlooker.utils.addons.viamcp.viamcp.MCPVLBPipeline;
...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            try {
                ...
                Disabler dis = Client.INSTANCE.getModuleCollection().getModule(Disabler.class);
                if (p_channelRead0_2_ instanceof S3FPacketCustomPayload) {
                    ...
                    Client.INSTANCE.getEventProtocol().handleEvent(packetReceive);
                    if (packetReceive.isCancelled()) {
                        return;
                    }
                    packet.processPacket(this.packetListener);
                }
                else if (dis.getGrimPost() &&  dis.grimPostDelay(p_channelRead0_2_)) {
                    mc.addScheduledTask(() -> dis.storedPackets.add(packet));
                }
                else {
                    final PacketReceiveEvent packetReceive = new PacketReceiveEvent(p_channelRead0_2_);
                    Client.INSTANCE.getEventProtocol().handleEvent(packetReceive);
    ...
    public static NetworkManager provideLocalClient(SocketAddress address) {
        ...
        Executor executor = Executors.newCachedThreadPool(new DefaultThreadFactory("Netty Epoll Server IO", true));
        ...
        bootstrap.connect(address).addListener((ChannelFutureListener) future -> {
            if (future.isSuccess()) {
                ...
                logger.warn("Local connection attempt failed");

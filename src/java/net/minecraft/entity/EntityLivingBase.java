...
import dev.onlooker.event.impl.player.JumpFixEvent;
import dev.onlooker.event.impl.player.LivingDeathEvent;
import dev.onlooker.module.impl.world.PacketFix;
...
public abstract class EntityLivingBase extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        if (this.isPotionActive(Potion.digSpeed)) {
            ...
            return this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : Client.INSTANCE.getModuleCollection().get(Animations.class).isEnabled() ? 6 + (Animations.SwingSpeed.getValue().intValue()) * 2 : 6;
        }
    }
    ...
    public void onLivingUpdate() {
        ...
        if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() >= 47 && ((PacketFix) Client.INSTANCE.getModuleCollection().get(PacketFix.class)).fixMotionJump.getValue()) {

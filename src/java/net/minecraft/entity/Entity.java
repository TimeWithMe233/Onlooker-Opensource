...
import dev.onlooker.event.impl.player.PlayerMoveUpdateEvent;
import dev.onlooker.event.impl.player.SafeWalkEvent;
import dev.onlooker.event.impl.player.StepConfirmEvent;
import dev.onlooker.event.impl.player.StrafeEvent;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            final StrafeEvent event = new StrafeEvent(strafe, forward, friction);
    ...
    public float getCollisionBorderSize() {
        return ViaMCP.INSTANCE.getAsyncVersionSlider().id <= 47 ? 0.1F : 0.0F;
    }
    ...
    public dev.onlooker.utils.vector.Vector3d getCustomPositionVector() {
        return new dev.onlooker.utils.vector.Vector3d(posX, posY, posZ);
    }
